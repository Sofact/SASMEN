...
$(document).ready(function(){
	...
	var usuario = '<%=request.getUserPrincipal().getName()%>';
			...
			$.ajax({
			type: 'GET',
			url: 'ServletCargaConvenioFiltrado',
			data: 'i='+usuario,
			statusCode: {
				404: function(){
...
$(document).ready(function(){
	...
	var convId = document.getElementById("oculta").value;
    ...
    .H3t
{
 color: #3366CC;
  ...
  <h3 class="H3">Herramienta de Apoyo a la supervisi&oacuten MEN</h3>
 	<div><label id="user" name= "user""><%=request.getUserPrincipal().getName()%></label></div>
  ...
  <div class="row">
    <div class="col">
      <h3 class="H3t">Actualizaci&oacuten de Convenios</h3>
		<label for="Agregar Nombre Convenio" class="form-label">Seleccione Convenio</label>
			<select class="form-control" id="nombreConvenio" name="nombreConvenio">
		...
		<label hidden id='oculta' name= 'oculta'></label>
		      ...
		      <div class="accordion-body">
				  ...
				  <input type="text" aria-label="numeroConvenio" id="numeroConvenio" name ="numeroConvenio" class="form-control">
