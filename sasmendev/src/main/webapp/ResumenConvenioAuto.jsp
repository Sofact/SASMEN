	...
	$('div[name=alarmaFecha]').empty();
		$.ajax({
			...
			success: function(res){
				...
				if (meses < 7 && meses > 1)
				{
				$('div[name=alarmaFecha]').append("<div class='alert alert-danger' role='alert' style='color:red'><strong>!!Faltan ("+Math.round(meses)+") meses días para la terminación del convenio¡¡  </strong>   <i class='fa fa-bell' style='font-size:36px;color:red'></i></div>");
				}
				if (meses <= 1 && meses >= 0)
				{
  ...
  <div class="row">
    <div class="col">
      <h3 class="H3t">Resumen convenio</h3>
		...
		<div class="progress">
		  <div class="progress-bar" role="progressbar" id="progresoTotal" name="progresoTotal"   style="width:100%;height:40px; font-size: 65px;" aria-valuenow="50" aria-valuemin="0" aria-valuemax="100">0%</div>
