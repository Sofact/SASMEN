...
/**
 * Servlet implementation class ServletCalculaAvanceConvenio
 */
@WebServlet("/ServletCalculaAvanceConvenio")
public class ServletCalculaAvanceConvenio extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ServletCalculaAvanceConvenio() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int val = Integer.parseInt(request.getParameter("convenio"));
		int convId = 0;
		...
		List<SgdConvenioTipoDocumento> cotido = gd.getConvenioTipoDocumento(val);
		List<SgdDocumento> documento = gd.getDocumento(val);
		List<SgdDocumento> documentoAperiodico = gd.getDocumentoAperiodico(val);
		...
		StringBuilder sb= new StringBuilder("");
			...
			if (documento.size()==1) {
				...
				sb.append("total"+"|"+global + "|" + Math.ceil(totalDocumentos)+"||");
				sb.append("subtitulo"+"|"+documento.get(0).getDocuNombre() + " "+documento.get(0).getEnlaId()+ "|" + totalDocumentos + "|" + cantidadTotalByDoc+"||");
				sb.append("contenido"+"|"+documento.get(0).getDocuNombre()+"|"+documento.get(0).getDocuFechaIni()+"|"+documento.get(0).getDocuFechaFin()+"|"+documento.get(0).getDocuPath()+"|"+documento.get(0).getDocuId()+"|"+cotido.get(0).getCotdPeriodicidad()+"||");
